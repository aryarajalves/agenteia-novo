...
async def test_toggle_agent(client: AsyncClient):
    ...
    response = await client.post(f"/agents/{agent_id}/toggle_status")
    assert response.status_code == 200
...
async def test_generate_agent_description(client: AsyncClient):
    create_res = await client.post("/agents", json={
        "name": "Generate Description Me", 
    ...
    with patch("openai.AsyncOpenAI") as MockOpenAI:
        ...
        mock_response.choices[0].message.content = "This is a mock description generated by AI."
        mock_instance.chat.completions.create = AsyncMock(return_value=mock_response)

        with patch("os.getenv", side_effect=mock_getenv):
            response = await client.post(f"/agents/{agent_id}/generate-description")
            
        assert response.status_code == 200
        assert response.json()["description"] == "This is a mock description generated by AI."
